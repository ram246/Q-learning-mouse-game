...
#define numFeatures 8			// UPDATE THIS to be the number of features you have
#define maxObjects 5
...
void maxQsaHelper(double gr[max_graph_size][4],double weights[25],int mouse_pos[1][2], int cats[5][2], int cheeses[5][2], int size_X, int graph_size, double *maxU, int catId, int *init);

double Dikstras(double gr[max_graph_size][4], int size_X, int agent[1][2], int agentNumber);
struct PQNode {
  int point[2];
  int priority;
};
